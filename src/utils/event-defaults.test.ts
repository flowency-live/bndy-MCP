...
import { defaultStartTime, resolveStartTime, DEFAULT_END_TIME } from './event-defaults.js';
...
describe('create_event contract', () => {
  test('startTime is optional, so an agent never has to ask for it', () => {
    // resolveStartTime accepts undefined without throwing. That is the whole
    // point: a missing time is normal input, not an error condition.
    expect(() => resolveStartTime('2026-08-15', undefined)).not.toThrow();
  });

  test('end time defaults to midnight', () => {
